...
class CheeseViewModel(app: Application) : AndroidViewModel(app) {
    private val db = CheeseDb.get(app)
    private val dao = db.cheeseDao()
    ...
    fun initFillDb(releaser: suspend () -> Unit) = launch {
        CHEESE_DATA.reversed().chunked(30).map {
            releaser()
            ioThread { dao.insert(it.map { Cheese(id = 0, name = it) }) }
        }
    }

    fun clearDb() {
        ioThread { db.clearAllTables() }
    }
}